...
class OrderItem < ActiveRecord::Base
 ...
 # @return [Boolean]
  ...
  def shipping_rate_options(total_charge)
    ShippingRate.joins(:shipping_method).where(['shipping_rates.minimum_charge <= ?', total_charge])
  end
  ...
  # @return [OrderItem] Object has addional attributes of [sum_price, sum_total, and quantity]
  def self.order_items_in_cart(order_id)
    joins({:variant => :product }).
    where({ :order_items => { :order_id => order_id}}).
    select( " order_items.id, order_items.order_id, order_items.shipping_rate_id, 
              order_items.state, order_items.tax_rate_id, order_items.price, 
              order_items.total, order_items.variant_id, count(*) as quantity, 
              SUM(order_items.price) as sum_price, SUM(order_items.total) as sum_total").
